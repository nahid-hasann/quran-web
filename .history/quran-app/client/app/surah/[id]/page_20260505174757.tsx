...
export default function SurahPage() {
    ...
    const playAudio = (ayahId: number) => {
        if (currentAudio && currentPlayingId === ayahId) {
        ...
        currentAudio = new Audio(`https://cdn.islamic.network/quran/audio/128/ar.alafasy/${ayahId}.mp3`);
        currentPlayingId = ayahId;
        setPlayingId(ayahId);
        currentAudio.play();
    ...
    useEffect(() => {
        if (id) {
            // CRITICAL: Oboshoy backticks (``) use koro, quotes ('') na.
            fetch(`http://localhost:5001/api/surah/${id}`)
                .then((res) => {
                    if (!res.ok) throw new Error("Surah not found in database");
                    return res.json();
                })
                .then((data) => {
                    setSurah(data);
    ...
    if (!surah) return <div className="p-10 text-center text-gray-500">Loading Surah {id}... Please ensure backend is running and seeded.</div>;
                        ...
                        <div className="flex flex-col space-y-10">
                            <div className="flex justify-between items-start gap-6">
                                <div className="flex flex-col items-center gap-4">
                                    ...
                                    <button
                                        onClick={() => playAudio(ayah.id)}
                                        className={`w-14 h-14 flex items-center justify-center rounded-full text-2xl ${playingId === ayah.id ? "bg-green-600 text-white" : "bg-gray-800 text-gray-400"}`}
                                    >
                                        {playingId === ayah.id ? "⏸" : "▶️"}
                                    </button>
                                </div>
